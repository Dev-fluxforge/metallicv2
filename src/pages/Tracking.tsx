        ...
        >
          <form className="space-y-6" onSubmit={handleTrack}>
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-widest font-bold text-white/40 ml-4">Order ID</label>
              <div className="relative">
                <Package className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20" />
                <input 
                  type="text" 
                  placeholder="MH-123456" 
                  value={orderId}
                  onChange={(e) => setOrderId(e.target.value)}
                  className="w-full bg-brand-dark border border-brand-line rounded-2xl pl-16 pr-6 py-5 focus:outline-none focus:border-brand-primary transition-colors" 
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="rider@example.com" className="w-full bg-brand-dark border border-brand-line rounded-2xl pl-16 pr-6 py-5 focus:outline-none focus:border-brand-primary transition-colors" />
              </div>
            </div>
          ...
          >
            <div className="bg-brand-surface border border-brand-line rounded-[3rem] overflow-hidden">
              <div className="p-8 md:p-12 border-b border-brand-line flex flex-col md:flex-row justify-between items-center gap-8">
                <div>
                  <p className="text-[10px] uppercase tracking-widest font-bold text-white/40 mb-2">Current Status</p>
                  <h3 className="text-3xl text-brand-primary">In Transit</h3>
                </div>
                <div className="text-right">
                  <p className="text-[10px] uppercase tracking-widest font-bold text-white/40 mb-2">Estimated Delivery</p>
                  ...
                  <div className="relative flex flex-col md:flex-row justify-between gap-12">
                    {[
                      ...
                      <div key={i} className="flex md:flex-col items-center gap-6 md:gap-8 text-center">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center z-10 border-4 border-brand-surface ${step.active ? 'bg-brand-primary text-brand-dark' : 'bg-brand-dark text-white/20 border-brand-line'}`}>
                          <step.icon className="w-6 h-6" />
                        </div>
                        <div className="text-left md:text-center">
                          <p className={`font-bold uppercase tracking-widest text-xs mb-1 ${step.active ? 'text-white' : 'text-white/20'}`}>{step.label}</p>
              ...
              <div className="p-8 md:p-12 bg-brand-dark/50 border-t border-brand-line">
